...
zpool_get_errlog_process_obj_group(zpool_handle_t *zhp, zbookmark_phys_t *zb,
	...
	block_ids = zfs_alloc(zhp->zpool_hdl, error_count * sizeof(*block_ids));
        indrt_levels = zfs_alloc(zhp->zpool_hdl, error_count * sizeof (*indrt_levels));
...
zpool_get_errlog(zpool_handle_t *zhp, nvlist_t **nverrlistp)
{
	...
	for (i = 0; i < count;) {
		...
		i += zpool_get_errlog_process_obj_group(zhp, &zb[i], count - i, nv);
