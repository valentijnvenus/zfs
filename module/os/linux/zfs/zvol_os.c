...
#define	ZVOL_BIO_END_IO(zv, bio, error)	BIO_END_IO(bio, error, \
    zv->zv_zso->use_blk_mq ? 0 : 1)
...
static kmem_cache_t *blk_mq_cache = NULL;
...
struct zvol_state_os {
	...
	/*
	 * lock to protect the BIO counter.  The BIO counter is a per-request
	 * counter that is set to the total number of BIOs in the request, and
	 * then gets decremented as the BIOs in the request complete.  The last
	 * completed BIO then calls blk_mq_end_request() to tell the kernel the
	 * request is done.  We need a lock to ensure only one BIO is
	 * decrementing the counter at a time.
	 */
	spinlock_t	zvo_blkmq_bio_counter_lock;
#endif
...
#ifdef HAVE_BLK_MQ
/*
 * Given a struct request, look at all the statuses of its completed BIOs and
 * return the first error seen (or BLK_STS_OK if no errors).
 */
static blk_status_t
blk_mq_get_request_status_from_bios(struct request *rq)
{
	struct bio *bio;
 ...
 * we process all the BIOs in a request.
 */
static void zvol_mq_work_func(struct work_struct *work)
{
	zv_work_t *zv_work = container_of(work, zv_work_t, work);
	struct request *rq = zv_work->rq;
	zvol_state_t *zv = rq->q->queuedata;
	struct bio *bio = zv_work->bio;
	...
	spin_lock(&zso->zvo_blkmq_bio_counter_lock);
	count = (unsigned long) rq->end_io_data;
	if (count > 0) {
		count--;
		if (count == 0) {
			all_done = 1;
		}
		rq->end_io_data = (void *) count;
	}

	spin_unlock(&zso->zvo_blkmq_bio_counter_lock);

	/* All done */
	if (all_done) {
		res = blk_mq_get_request_status_from_bios(rq);
 ...
 * contains one or more BIOs.  This function is run from an atomic context.
 */
    ...
    const struct blk_mq_queue_data *bd)
{
	...
	count = blk_rq_count_bios(rq);
	rq->end_io_data = (void *) count;

	__rq_for_each_bio(bio, rq) {
		/*
		 * Add the BIOs to our workqueue for processing later since
		 * we're currently in an atomic context and our bio processing
		 * code can sleep.
		 */
		zv_work = kmem_cache_alloc(blk_mq_cache, KM_NOSLEEP);
		if (!zv_work) {
			/* Try again, maybe memory will free up */
			return (BLK_STS_AGAIN);
		}

		INIT_WORK(&zv_work->work, zvol_mq_work_func);
		zv_work->rq = rq;
		zv_work->bio = bio;
		schedule_work(&zv_work->work);
	}
...
/*
 * When HAVE_BLK_MQ is defined accounting is done by blk_mq_start_request()
 * and blk_mq_end_request() in the zvol_mq_work_func() function.
 */
...
zvol_request_impl(zvol_state_t *zv, struct bio *bio, boolean_t force_sync)
{
	...
	if (bio_has_data(bio) && offset + size > zv->zv_volsize) {
		...
		ZVOL_BIO_END_IO(zv, bio, -SET_ERROR(EIO));
		goto out;
	}
	...
	if (rw == WRITE) {
		if (unlikely(zv->zv_flags & ZVOL_RDONLY)) {
			ZVOL_BIO_END_IO(zv, bio, -SET_ERROR(EROFS));
			goto out;
		}
...
zvol_alloc_blk_mq(zvol_state_t *zv)
{
	...
	spin_lock_init(&zso->zvo_blkmq_bio_counter_lock);
...
zvol_alloc(dev_t dev, const char *name)
{
	...
	/*
	 * Just to decode this #ifdef rat's nest a little...
	 *
...
zvol_init(void)
{
	...
	/* Create a kmem cache for all our blk-mq work items */
	blk_mq_cache = kmem_cache_create("blk_mq_cache",
	    sizeof (zv_work_t), 0, NULL, NULL, NULL, NULL, NULL, 0);
	if (!blk_mq_cache) {
...
zvol_fini(void)
{
	...
	unregister_blkdev(zvol_major, ZVOL_DRIVER);
#ifdef HAVE_BLK_MQ
	kmem_cache_destroy(blk_mq_cache);
#endif
