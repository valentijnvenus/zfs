...
#define	ZVOL_BIO_END_IO(zv, bio, rq, error)	do { \
		BIO_END_IO(bio, error, zv->zv_zso->use_blk_mq ? 0 : 1); \
		if (zv->zv_zso->use_blk_mq) \
			zvol_blk_mq_bio_end_io(zv, rq); \
	} while (0);
...
static void
zvol_blk_mq_bio_end_io(zvol_state_t *zv, struct request *rq)
{
 ...
 */
	...
	ZVOL_BIO_END_IO(zv, bio, rq, -error);
}
    ...
    boolean_t force_sync)
{
	...
	if (bio_has_data(bio) && offset + size > zv->zv_volsize) {
		...
		ZVOL_BIO_END_IO(zv, bio, rq, -SET_ERROR(EIO));
		goto out;
	}
	...
	if (rw == WRITE) {
		if (unlikely(zv->zv_flags & ZVOL_RDONLY)) {
			ZVOL_BIO_END_IO(zv, bio, rq, -SET_ERROR(EROFS));
			goto out;
		}
